...
namespace LunarPlugin
{
    ...
    public abstract class CCommand : IComparable<CCommand>
    {
        ...
        internal string AutoComplete(string commandLine, IList<string> tokens, bool doubleTab)
        {
            Iterator<string> iter = new Iterator<string>(tokens);
            iter.Next(); // first token is a command name
        ...
        private string AutoCompleteOption(string commandLine, Iterator<string> iter, string optNameToken, string prefix, bool doubleTab)
        {
        ...
        private string AutoCompleteOption(string commandLine, string optToken, string argToken, bool doubleTab, bool useShort)
        {
            string prefix = useShort ? "-" : "--";
            string optName = optToken.Length > prefix.Length ? optToken.Substring(prefix.Length) : null;

            // list options
            IList<Option> optionsList;
            if (useShort)
            {
        ...
        private string AutoComplete(string commandLine, Option opt, string token, bool doubleTab)
        {
            if (token != null)
            {
                if (opt.Values != null)
        ...
        protected virtual string AutoCompleteArgs(string commandLine, string token, bool doubleTab)
        {
        ...
        private string AutoCompleteArgs(string commandLine, string[] values, string token, bool doubleTab)
        {
        ...
        private void PrintOptions(string commandLine, IList<Option> options, bool useShort)
        {
            if (options.Count > 0)
            {
                PrintPrompt(commandLine);
        ...
        private string GetSuggestedText(string token, IList<Option> options, bool useShort)
        {
            if (options.Count == 1)
            {
