...
#ifndef LUGCDAsyncSocketLoggingEnabled
#define LUGCDAsyncSocketLoggingEnabled 0
#endif

#if LUGCDAsyncSocketLoggingEnabled
...
#define LogAsync   YES
#define LogContext LUGCDAsyncSocketLoggingContext
...
#ifndef LUGCDAsyncSocketLogLevel
#define LUGCDAsyncSocketLogLevel LOG_LEVEL_VERBOSE
#endif

// Log levels : off, error, warn, info, verbose
static const int logLevel = LUGCDAsyncSocketLogLevel;
...
NSString *const LUGCDAsyncSocketException = @"LUGCDAsyncSocketException";
NSString *const LUGCDAsyncSocketErrorDomain = @"LUGCDAsyncSocketErrorDomain";

NSString *const LUGCDAsyncSocketQueueName = @"LUGCDAsyncSocket";
NSString *const LUGCDAsyncSocketThreadName = @"LUGCDAsyncSocket-CFStream";

NSString *const LUGCDAsyncSocketManuallyEvaluateTrust = @"LUGCDAsyncSocketManuallyEvaluateTrust";

enum LUGCDAsyncSocketFlags
{
...
@interface LUGCDAsyncSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
@implementation LUGCDAsyncSocketPreBuffer
...
/**
 * The LUGCDAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
**/
@interface LUGCDAsyncReadPacket : NSObject
{
  @public
...
- (NSUInteger)readLengthForTermWithPreBuffer:(LUGCDAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;

- (NSInteger)searchForTermAfterPreBuffering:(ssize_t)numBytes;

@end

@implementation LUGCDAsyncReadPacket
...
/**
 * The LUGCDAsyncWritePacket encompasses the instructions for any given write.
**/
@interface LUGCDAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation LUGCDAsyncWritePacket
...
@implementation LUGCDAsyncSocket
{
	uint32_t flags;
	uint16_t config;
	...
	LUGCDAsyncReadPacket *currentRead;
	LUGCDAsyncWritePacket *currentWrite;
	
	unsigned long socketFDBytesAvailable;
	
	LUGCDAsyncSocketPreBuffer *preBuffer;
...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		...
		{
			socketQueue = dispatch_queue_create([LUGCDAsyncSocketQueueName UTF8String], NULL);
		}
		...
		preBuffer = [[LUGCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
	}
	return self;
}
...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (enableIPv4)
		{
			...
			__weak LUGCDAsyncSocket *weakSelf = self;
			
			dispatch_source_set_event_handler(accept4Source, ^{ @autoreleasepool {
			#pragma clang diagnostic push
			#pragma clang diagnostic warning "-Wimplicit-retain-self"
				
				__strong LUGCDAsyncSocket *strongSelf = weakSelf;
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			// Create LUGCDAsyncSocket instance for accepted socket
			
			LUGCDAsyncSocket *acceptedSocket = [[LUGCDAsyncSocket alloc] initWithDelegate:theDelegate
                ...
                error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		#pragma clang diagnostic warning "-Wimplicit-retain-self"
			...
			NSMutableArray *addresses = [LUGCDAsyncSocket lookupHost:hostCpy port:port error:&lookupErr];
			...
			else
			{
				...
				{
					if (!address4 && [LUGCDAsyncSocket isIPv4Address:address])
					{
						address4 = address;
					}
					else if (!address6 && [LUGCDAsyncSocket isIPv6Address:address])
					{
						address6 = address;
					}
				}
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:LUGCDAsyncSocketErrorDomain code:LUGCDAsyncSocketBadConfigError userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:LUGCDAsyncSocketErrorDomain code:LUGCDAsyncSocketBadParamError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"LUGCDAsyncSocketConnectTimeoutError",
	                                                     @"LUGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Attempt to connect to host timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:LUGCDAsyncSocketErrorDomain code:LUGCDAsyncSocketConnectTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"LUGCDAsyncSocketReadMaxedOutError",
														 @"LUGCDAsyncSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:LUGCDAsyncSocketErrorDomain code:LUGCDAsyncSocketReadMaxedOutError userInfo:info];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"LUGCDAsyncSocketReadTimeoutError",
	                                                     @"LUGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Read operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:LUGCDAsyncSocketErrorDomain code:LUGCDAsyncSocketReadTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"LUGCDAsyncSocketWriteTimeoutError",
	                                                     @"LUGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Write operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:LUGCDAsyncSocketErrorDomain code:LUGCDAsyncSocketWriteTimeoutError userInfo:userInfo];
}

- (NSError *)connectionClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"LUGCDAsyncSocketClosedError",
	                                                     @"LUGCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Socket closed by remote peer", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:LUGCDAsyncSocketErrorDomain code:LUGCDAsyncSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:LUGCDAsyncSocketErrorDomain code:LUGCDAsyncSocketOtherError userInfo:userInfo];
}
                        ...
                        tag:(long)tag
{
	...
	LUGCDAsyncReadPacket *packet = [[LUGCDAsyncReadPacket alloc] initWithData:buffer
	                                                          startOffset:offset
	                                                            maxLength:length
	                                                              timeout:timeout
	                                                           readLength:0
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		...
		if (!currentRead || ![currentRead isKindOfClass:[LUGCDAsyncReadPacket class]])
		{
...
- (void)maybeDequeueRead
{
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
            ...
            LogVerbose(@"Dequeued LUGCDAsyncReadPacket");
...
- (void)doReadData
{
	...
	if ([self usingCFStreamForTLS])
	{
		...
		// Requested CFStream, rather than SecureTransport, for TLS (via LUGCDAsyncSocketUseCFStreamForTLS)
...
- (void)completeCurrentRead
{
	...
	{
		LUGCDAsyncReadPacket *theRead = currentRead; // Ensure currentRead retained since result may not own buffer
...
- (void)doReadTimeout
{
	...
	{
		LUGCDAsyncReadPacket *theRead = currentRead;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	LUGCDAsyncWritePacket *packet = [[LUGCDAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		...
		if (!currentWrite || ![currentWrite isKindOfClass:[LUGCDAsyncWritePacket class]])
		{
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		if ([writeQueue count] > 0)
		{
            ...
            LogVerbose(@"Dequeued LUGCDAsyncWritePacket");
...
- (void)doWriteData
{
	...
	// Note: This method is not called if currentWrite is a LUGCDAsyncSpecialPacket (startTLS packet)
...
- (void)doWriteTimeout
{
	...
	{
		LUGCDAsyncWritePacket *theWrite = currentWrite;
...
+ (void)startCFStreamThreadIfNeeded
{
	...
	dispatch_once(&predicate, ^{
		...
		cfstreamThreadSetupQueue = dispatch_queue_create("LUGCDAsyncSocket-CFStreamThreadSetup", DISPATCH_QUEUE_SERIAL);
	});
...
+ (void)stopCFStreamThreadIfNeeded
{
	...
	#pragma clang diagnostic warning "-Wimplicit-retain-self"
		...
		if (--cfstreamThreadRetainCount == 0)
		{
			...
			// wake up the thread
			[LUGCDAsyncSocket performSelector:@selector(ignore:)
...
{
	[[NSThread currentThread] setName:LUGCDAsyncSocketThreadName];
...
+ (void)scheduleCFStreams:(LUGCDAsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(LUGCDAsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	LUGCDAsyncSocket *asyncSocket = (__bridge LUGCDAsyncSocket *)pInfo;
