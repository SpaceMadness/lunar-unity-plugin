...
extern NSString *const LUGCDAsyncUdpSocketException;
extern NSString *const LUGCDAsyncUdpSocketErrorDomain;

extern NSString *const LUGCDAsyncUdpSocketQueueName;
extern NSString *const LUGCDAsyncUdpSocketThreadName;

enum LUGCDAsyncUdpSocketError
{
	LUGCDAsyncUdpSocketNoError = 0,          // Never used
	LUGCDAsyncUdpSocketBadConfigError,       // Invalid configuration
	LUGCDAsyncUdpSocketBadParamError,        // Invalid parameter was passed
	LUGCDAsyncUdpSocketSendTimeoutError,     // A send operation timed out
	LUGCDAsyncUdpSocketClosedError,          // The socket was closed
	LUGCDAsyncUdpSocketOtherError,           // Description provided in userInfo
};
typedef enum LUGCDAsyncUdpSocketError LUGCDAsyncUdpSocketError;
 ...
 * Example:
 * 
 * LUGCDAsyncUdpSocketReceiveFilterBlock filter = ^BOOL (NSData *data, NSData *address, id *context) {
 * 
 ...
 * 
**/
typedef BOOL (^LUGCDAsyncUdpSocketReceiveFilterBlock)(NSData *data, NSData *address, id *context);
 ...
 *
**/
typedef BOOL (^LUGCDAsyncUdpSocketSendFilterBlock)(NSData *data, NSData *address, long tag);


@interface LUGCDAsyncUdpSocket : NSObject

/**
 * LUGCDAsyncUdpSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, LUGCDAsyncSocket will automatically create its own socket queue.
 ...
 * 
 * This means LUGCDAsyncUdpSocket automatically supports both protocols,
 * and can send to IPv4 or IPv6 addresses,
 * as well as receive over IPv4 and IPv6.
 * 
 * For operations that require DNS resolution, LUGCDAsyncUdpSocket supports both IPv4 and IPv6.
 * If a DNS lookup returns only IPv4 results, LUGCDAsyncUdpSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, LUGCDAsyncUdpSocket will automatically use IPv6.
 ...
 * This is due to the fact that LUGCDAsyncUdpSocket does NOT copy the data.
 ...
 *    or the system tools to handle this aren't available (e.g. on a mobile device).
 * 
 * For more information about LUGCDAsyncUdpSocketSendFilterBlock, see the documentation for its typedef.
...
**/
- (void)setSendFilter:(LUGCDAsyncUdpSocketSendFilterBlock)filterBlock withQueue:(dispatch_queue_t)filterQueue;
...
**/
- (void)setSendFilter:(LUGCDAsyncUdpSocketSendFilterBlock)filterBlock
 ...
 * 
 * Important Note:
 * LUGCDAsyncUdpSocket may be running in parallel with your code.
 * That is, your delegate is likely running on a separate thread/dispatch_queue.
 * When you invoke this method, LUGCDAsyncUdpSocket may have already dispatched delegate methods to be invoked.
 ...
 * [udpSocket setReceiveFilter:filter withQueue:myParsingQueue];
 * 
 * For more information about LUGCDAsyncUdpSocketReceiveFilterBlock, see the documentation for its typedef.
 * To remove a previously set filter, invoke this method and pass a nil filterBlock and NULL filterQueue.
 * 
 * Note: This method invokes setReceiveFilter:withQueue:isAsynchronous: (documented below),
 *       passing YES for the isAsynchronous parameter.
**/
- (void)setReceiveFilter:(LUGCDAsyncUdpSocketReceiveFilterBlock)filterBlock withQueue:(dispatch_queue_t)filterQueue;
 ...
 * For example, you can't query properties on the socket.
**/
- (void)setReceiveFilter:(LUGCDAsyncUdpSocketReceiveFilterBlock)filterBlock
               withQueue:(dispatch_queue_t)filterQueue
          isAsynchronous:(BOOL)isAsynchronous;
 ...
 * 
 * The LUGCDAsyncUdpSocket instance may optionally be used again.
 ...
 * 
 * Generally LUGCDAsyncUdpSocket doesn't use CFStream. (It uses the faster GCD API's.)
...
@protocol LUGCDAsyncUdpSocketDelegate
@optional
...
**/
- (void)udpSocket:(LUGCDAsyncUdpSocket *)sock didConnectToAddress:(NSData *)address;
...
**/
- (void)udpSocket:(LUGCDAsyncUdpSocket *)sock didNotConnect:(NSError *)error;
