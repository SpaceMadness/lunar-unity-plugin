...
@class LUGCDAsyncUdpSendPacket;

NSString *const LUGCDAsyncUdpSocketException = @"LUGCDAsyncUdpSocketException";
NSString *const LUGCDAsyncUdpSocketErrorDomain = @"LUGCDAsyncUdpSocketErrorDomain";

NSString *const LUGCDAsyncUdpSocketQueueName = @"LUGCDAsyncUdpSocket";
NSString *const LUGCDAsyncUdpSocketThreadName = @"LUGCDAsyncUdpSocket-CFStream";

enum LUGCDAsyncUdpSocketFlags
{
...
enum LUGCDAsyncUdpSocketConfig
{
...
@interface LUGCDAsyncUdpSocket ()
{
	...
	LUGCDAsyncUdpSocketReceiveFilterBlock receiveFilterBlock;
	dispatch_queue_t receiveFilterQueue;
	BOOL receiveFilterAsync;
	
	LUGCDAsyncUdpSocketSendFilterBlock sendFilterBlock;
	...
	LUGCDAsyncUdpSendPacket *currentSend;
...
/**
 * The LUGCDAsyncUdpSendPacket encompasses the instructions for a single send/write.
**/
@interface LUGCDAsyncUdpSendPacket : NSObject {
@public
	NSData *buffer;
...
@implementation LUGCDAsyncUdpSendPacket
...
@interface LUGCDAsyncUdpSpecialPacket : NSObject {
@public
//	uint8_t type;
...
@implementation LUGCDAsyncUdpSpecialPacket
...
@implementation LUGCDAsyncUdpSocket
...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	...
	if ((self = [super init]))
	{
		...
		{
			socketQueue = dispatch_queue_create([LUGCDAsyncUdpSocketQueueName UTF8String], NULL);
		}
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:LUGCDAsyncUdpSocketErrorDomain
	                           code:LUGCDAsyncUdpSocketBadConfigError
	                       userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:LUGCDAsyncUdpSocketErrorDomain
	                           code:LUGCDAsyncUdpSocketBadParamError
	                       userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"LUGCDAsyncUdpSocketSendTimeoutError",
	                                                     @"LUGCDAsyncUdpSocket", [NSBundle mainBundle],
	                                                     @"Send operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:LUGCDAsyncUdpSocketErrorDomain
	                           code:LUGCDAsyncUdpSocketSendTimeoutError
	                       userInfo:userInfo];
}

- (NSError *)socketClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"LUGCDAsyncUdpSocketClosedError",
	                                                     @"LUGCDAsyncUdpSocket", [NSBundle mainBundle],
	                                                     @"Socket closed", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:LUGCDAsyncUdpSocketErrorDomain code:LUGCDAsyncUdpSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:LUGCDAsyncUdpSocketErrorDomain
	                           code:LUGCDAsyncUdpSocketOtherError
	                       userInfo:userInfo];
}
...
- (BOOL)connectToHost:(NSString *)host onPort:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		LUGCDAsyncUdpSpecialPacket *packet = [[LUGCDAsyncUdpSpecialPacket alloc] init];
...
- (void)maybeConnect
{
	...
	BOOL sendQueueReady = [currentSend isKindOfClass:[LUGCDAsyncUdpSpecialPacket class]];
	
	if (sendQueueReady)
	{
		LUGCDAsyncUdpSpecialPacket *connectPacket = (LUGCDAsyncUdpSpecialPacket *)currentSend;
...
- (void)sendData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	LUGCDAsyncUdpSendPacket *packet = [[LUGCDAsyncUdpSendPacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)sendData:(NSData *)data toAddress:(NSData *)remoteAddr withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	LUGCDAsyncUdpSendPacket *packet = [[LUGCDAsyncUdpSendPacket alloc] initWithData:data timeout:timeout tag:tag];
	packet->addressFamily = [LUGCDAsyncUdpSocket familyFromAddress:remoteAddr];
...
{
	LUGCDAsyncUdpSocketSendFilterBlock newFilterBlock = NULL;
...
- (void)maybeDequeueSend
{
	...
	if (currentSend == nil)
	{
		...
		while ([sendQueue count] > 0)
		{
			...
			if ([currentSend isKindOfClass:[LUGCDAsyncUdpSpecialPacket class]])
			{
				[self maybeConnect];
...
- (void)doPreSend
{
	...
	if (sendFilterBlock && sendFilterQueue)
	{
		...
		if (sendFilterAsync)
		{
			...
			LUGCDAsyncUdpSendPacket *sendPacket = currentSend;
...
- (void)setReceiveFilter:(LUGCDAsyncUdpSocketReceiveFilterBlock)filterBlock
          ...
          isAsynchronous:(BOOL)isAsynchronous
{
	LUGCDAsyncUdpSocketReceiveFilterBlock newFilterBlock = NULL;
	dispatch_queue_t newFilterQueue = NULL;
...
+ (void)listenerThread
{
	@autoreleasepool {
		...
		[[NSThread currentThread] setName:LUGCDAsyncUdpSocketThreadName];
...
+ (void)addStreamListener:(LUGCDAsyncUdpSocket *)asyncUdpSocket
{
	LogTrace();
...
+ (void)removeStreamListener:(LUGCDAsyncUdpSocket *)asyncUdpSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == listenerThread, @"Invoked on wrong thread");
...
{
	@autoreleasepool {
		LUGCDAsyncUdpSocket *asyncUdpSocket = (__bridge LUGCDAsyncUdpSocket *)pInfo;
